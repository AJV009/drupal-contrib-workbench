...
See `CLAUDE.local.md` (generated by `install.sh`) for the drupal.org username
tied to this checkout. Treat that user as the person running the current
session unless the prompt says otherwise.
...
### Git & SSH
- drupal.org serves GitLab at two hostnames: `git.drupalcode.org` (HTTPS +
  GitLab UI) and `git.drupal.org` (SSH-only alias some contributors add to
  `~/.ssh/config`). Pick one and stay consistent across a checkout. If you
  add a `git.drupal.org` alias locally, prefer it for SSH remotes because
  it is shorter; otherwise `git.drupalcode.org` works everywhere out of the
  box.
...
- When testing AI features in any Drupal AI modules (e.g., `ai`, `ai_agent`,
  `mcp`, etc.), use whichever provider the user has a key for. The repo
  assumes Anthropic by default because `install.sh` writes an
  `anthropic.key`, but nothing in the workflow requires it — if a different
  provider key is on disk, use that.
- The provider key lives at `anthropic.key` in the workspace root (or another
  `*.key` file if the user configured a different provider). Read it from
  there when configuring the provider module or setting up keys for tests
  and usage. All `*.key` files are gitignored.
- These are local test instances, so paste the key into the provider UI,
  inject it via `ddev drush config:set`, put it in config YAML, or hardcode
  it wherever is most convenient — there is no production-grade secrets
  concern here.
